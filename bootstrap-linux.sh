...
# Make executable once: chmod +x bootstrap-linux.sh
# Then run: ./bootstrap-linux.sh
#
# This script is for first-time human setup and manual local runs.
# Do NOT point a launchd/systemd/supervisor entry at this file —
# production process managers should call the venv's python and
# app.py directly. See HANDOFF.md, "Production vs bootstrap scripts."
...
# ── Port safety (Rule 2) ──────────────────────────────────────────────────────
# Uses `ss` (iproute2) — present on virtually all modern Linux distros.
PORT=5000
